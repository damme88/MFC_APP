...
BEGIN_MESSAGE_MAP(MainFrame, CFrameWndEx)
  ...
  ON_COMMAND(ID_VIEW_SHOW_HIDE_PANEL, &MainFrame::OnShowHidePane)
END_MESSAGE_MAP()
...
int MainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
  ...
  if (!m_wndLocalBar.Create(_T("Macro Debugger"), this, CRect(0, 0, 300, 200), TRUE, ID_LOCAL_BAR,
                              WS_CHILD | WS_VISIBLE | CBRS_RIGHT | WS_CLIPSIBLINGS |
                              WS_CLIPCHILDREN | CBRS_RIGHT | CBRS_FLOAT_MULTI))
	{
		TRACE0("Failed to create Local Bar\n");
  ...
  m_wndLocalBar.EnableDocking(CBRS_ALIGN_ANY);
	EnableDocking(CBRS_ALIGN_ANY);
	DockPane(&m_wndMenuBar);
	DockPane(&m_wndToolBar);
  DockPane(&m_wndLocalBar);
...
void MainFrame::OnShowHidePane() {
  ShowPane(&m_wndLocalBar, !m_wndLocalBar.IsVisible(), FALSE, TRUE);
}